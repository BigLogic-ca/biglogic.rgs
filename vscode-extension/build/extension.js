...
class RGSWelcomeProvider {
    ...
    getChildren() {
        const items = [];
        // Header
        const header = new vscode.TreeItem('RGS (Argis) - Reactive Global State', vscode.TreeItemCollapsibleState.None);
        ...
        const docsAction = new vscode.TreeItem('Open Documentation', vscode.TreeItemCollapsibleState.None);
        docsAction.command = { command: 'vscode.open', title: 'Open Docs', arguments: ['https://github.com/BigLogic-ca/rgs'] };
        docsAction.contextValue = 'action';
        items.push(docsAction);
        // Info section
        const info = new vscode.TreeItem('Version: 3.8.2', vscode.TreeItemCollapsibleState.None);
...
class RGSWelcomeWebview {
    static show(context) {
        const panel = vscode.window.createWebviewPanel('rgsWelcome', 'RGS Welcome', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    getChildren(element) {
        ...
        if (!element) {
            ...
            if (this.analysis.stores.length > 0) {
                const storesFolder = new RGSTreeItem(`Stores (${this.analysis.stores.length})`, vscode.TreeItemCollapsibleState.Expanded, 'folder');
            ...
            if (this.analysis.states.length > 0) {
                const statesFolder = new RGSTreeItem(`State Usages (${this.analysis.states.length})`, vscode.TreeItemCollapsibleState.Expanded, 'folder');
                statesFolder.children = this.analysis.states.map(state => {
                    const item = new RGSTreeItem(state.key, vscode.TreeItemCollapsibleState.None, 'state');
            ...
            if (this.analysis.plugins.length > 0) {
                const pluginsFolder = new RGSTreeItem(`Plugins (${this.analysis.plugins.length})`, vscode.TreeItemCollapsibleState.Expanded, 'folder');
                pluginsFolder.children = this.analysis.plugins.map(plugin => {
                    const item = new RGSTreeItem(plugin.name, vscode.TreeItemCollapsibleState.None, 'plugin');
            ...
            if (this.analysis.violations.length > 0) {
                const violationsFolder = new RGSTreeItem(`Issues (${this.analysis.violations.length})`, vscode.TreeItemCollapsibleState.Expanded, 'folder');
                violationsFolder.children = this.analysis.violations.map(violation => {
                    const item = new RGSTreeItem(violation.message, vscode.TreeItemCollapsibleState.None, 'violation');
        ...
        if (element.itemType === 'store' && element.storeInfo) {
            ...
            if (config.namespace || config.persistByDefault || config.encryptionKey || config.auditEnabled) {
                const configItem = new RGSTreeItem('Configuration', vscode.TreeItemCollapsibleState.Expanded, 'folder');
                const configChildren = [];
                if (config.namespace)
                    configChildren.push(new RGSTreeItem('namespace: ' + config.namespace, vscode.TreeItemCollapsibleState.None, 'folder'));
                if (config.persistByDefault !== undefined)
                    configChildren.push(new RGSTreeItem('persistByDefault: ' + config.persistByDefault, vscode.TreeItemCollapsibleState.None, 'folder'));
                if (config.encryptionKey)
                    configChildren.push(new RGSTreeItem('encryption: enabled', vscode.TreeItemCollapsibleState.None, 'folder'));
                if (config.auditEnabled)
                    configChildren.push(new RGSTreeItem('auditEnabled: ' + config.auditEnabled, vscode.TreeItemCollapsibleState.None, 'folder'));
                if (config.accessRules && config.accessRules.length > 0)
                    configChildren.push(new RGSTreeItem('accessRules: ' + config.accessRules.length + ' rules', vscode.TreeItemCollapsibleState.None, 'folder'));
                if (config.sync)
                    configChildren.push(new RGSTreeItem('sync: enabled', vscode.TreeItemCollapsibleState.None, 'folder'));
                configItem.children = configChildren;
                children.push(configItem);
            }
            // Keys
            if (element.storeInfo.keys.length > 0) {
                const keysItem = new RGSTreeItem('Keys (' + element.storeInfo.keys.length + ')', vscode.TreeItemCollapsibleState.Expanded, 'folder');
                keysItem.children = element.storeInfo.keys.map(key => new RGSTreeItem(key, vscode.TreeItemCollapsibleState.None, 'folder'));
                children.push(keysItem);
            }
            // Plugins
            if (element.storeInfo.plugins.length > 0) {
                const pluginsItem = new RGSTreeItem('Plugins (' + element.storeInfo.plugins.length + ')', vscode.TreeItemCollapsibleState.Expanded, 'folder');
                pluginsItem.children = element.storeInfo.plugins.map(plugin => new RGSTreeItem(plugin, vscode.TreeItemCollapsibleState.None, 'folder'));
                children.push(pluginsItem);
            }
            // File location
            const locationItem = new RGSTreeItem('File: ' + path.basename(element.storeInfo.filePath) + ':' + element.storeInfo.line, vscode.TreeItemCollapsibleState.None, 'folder');
...
class WorkspaceAnalyzer {
    ...
    extractStoreConfig(content, matchIndex) {
        const config = {};
    ...
    findStoreKeys(content, storeName) {
...
class StoreDetailView {
    static register(context, treeProvider) {
        ...
        const showStoreDetails = async () => {
            ...
            if (!store)
                return;
            const panel = vscode.window.createWebviewPanel('rgsStoreDetails', 'Store: ' + store.name, vscode.ViewColumn.One, { enableScripts: true });
            const html = this.generateDetailHtml(store, treeProvider.analysis);
            panel.webview.html = html;
        };
        context.subscriptions.push(vscode.commands.registerCommand('rgs.showStoreDetails', showStoreDetails));
    }
    static generateDetailHtml(store, analysis) {
        const config = store.config;
        const relatedStates = analysis.states.filter((s) => store.keys.includes(s.key));
        return `<!DOCTYPE html>
<html>
<head>
  <style>
...
<body>
  ...
  <div class="section">
    ...
    ${store.keys.length > 0
            ? store.keys.map((k) => '<span class="key-badge">' + k + '</span>').join(' ')
  ...
  <div class="section">
    ...
    ${store.plugins.length > 0
            ? store.plugins.map((p) => '<span class="plugin-badge">' + p + '</span>').join(' ')
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('rgs.analyzeWorkspace', analyzeWorkspace));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('rgs.goToStoreDefinition', goToStoreDefinition));
    ...
    if (config.enableAutocomplete) {
        const completionProvider = vscode.languages.registerCompletionItemProvider(['typescript', 'typescriptreact', 'javascript', 'javascriptreact'], {
            provideCompletionItems(document, position, _token, _context) {
                ...
                const completionItems = completionsData.map(item => {
    ...
    const codeActionsProvider = vscode.languages.registerCodeActionsProvider(['typescript', 'typescriptreact', 'javascript', 'javascriptreact'], {
        provideCodeActions(document, range, context) {
            const actions = [];
            for (const diagnostic of context.diagnostics) {
                if (diagnostic.message.includes('RGS:') && diagnostic.code) {
                    const fix = new vscode.CodeAction('Apply suggestion: ' + diagnostic.code, vscode.CodeActionKind.QuickFix);
